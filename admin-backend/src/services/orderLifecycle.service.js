...
async function transitionOrder(order, to, { actorType = "system", actorId = null, note = null, session = null } = {}) {
  ...
  if (to === "return-requested" && note) update.$set.returnReason = note;
  ...
  if (updated === undefined) {
    ...
    if (to === "return-requested" && note) order.returnReason = note;
  ...
  try {
    ...
    if (user?.email) {
      const templates = {
        confirmed:"order-confirmation", shipped:"shipping",
        "out-for-delivery":"delivery", delivered:"delivery", cancelled:"cancellation",
        "return-requested":"return", returned:"return",
      };
      ...
      if (template) await sendTransactionalEmail(template,user.email,{
        ...
        status:to,reason:updated.cancelReason || updated.returnReason || note || ""
